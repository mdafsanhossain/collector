...
class ConnScraper : public IConnScraper {
 public:
  explicit ConnScraper(std::string proc_path, bool is_udp_collected, std::shared_ptr<ProcessStore> process_store = 0)
      : proc_path_(std::move(proc_path)), is_udp_collected_(is_udp_collected), process_store_(process_store) {}
